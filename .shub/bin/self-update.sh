...
REMOTE_VERSION=$(curl -s https://raw.githubusercontent.com/filipe1309/shubcogen-template/main/.shub/bin/version)
...
if [ "$REMOTE_VERSION" != "$LOCAL_VERSION" ]; then
        ...
        if [[ $response =~ ^(yes|y| ) ]] || [[ -z $response ]]; then
            echo "Updating..."
            curl -o .shub/bin/links.txt --create-dirs https://raw.githubusercontent.com/filipe1309/shubcogen-template/main/.shub/bin/links.txt
